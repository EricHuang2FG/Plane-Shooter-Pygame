...
class PlayerFighter:
    def __init__(self, speed, fireRate):
        ...
        self.picture = pygame.transform.scale(
            self.rawPicture,
            (self.rawPicture.get_width() * 0.25, self.rawPicture.get_height() * 0.25),
        )
    ...
    def move(self, dx, dy):
        if dy < 0:
            ...
            self.y = min(
                WINDOW.get_height() - self.picture.get_height(),
        ...
        if dx < 0:
            ...
            self.x = min(
                WINDOW.get_width() - self.picture.get_width(), self.x + dx * self.speed
            )
...
class PlayerBullet:
    ...
    def __init__(self, x, y, damage=1):
        ...
        self.picture = pygame.transform.scale(
            self.rawPicture,
            (self.rawPicture.get_width() * 0.1, self.rawPicture.get_height() * 0.1),
        )
...
class Enemy:
    def __init__(self, x, speed=1, hitpoints=2, scale=0.5):
        ...
        self.picture = pygame.transform.scale(
            self.rawPicture,
            (self.rawPicture.get_width() * scale, self.rawPicture.get_height() * scale),
        )
        self.width = self.picture.get_width()
        self.height = self.picture.get_height()
...
def rectangleIsColliding(r1, r2):
    ...
    return not (
        bl1[1] < tl2[1] or tl1[0] > tr2[0] or tl1[1] > bl2[1] or tr1[0] < tl2[0]
    )
    ...
    while run:
        ...
        if game == "start screen":
            ...
            if PLAYBUTTON.isClicked(mouse):
                ...
                bullets, enemies, score, enemyHealthDeduction, playerHealthDeduction = (
                    [],
                    [Enemy(WINDOW.get_width() / 2)],
                    [0],
                    {},
                    [None],
                )
